...
if (Use_local_data) {
   ...
   download.file('https://git.io/vXAZ1', destfile='./results/Result.RDS', mode='wb')
   ...
   download.file('https://git.io/vXgvv', destfile='./results/fuelling_model.RDS',mode='wb')
   ...
   download.file('https://git.io/vXEb5', destfile='./results/Fuelling_slopes.csv')
...
## Install package
Note that we used FLightR 0.4.7 version, and running this example in later versions may provide somehow different results.
...
The procedure of the definition of the twilight events, i.e. sunrises and sunsets, is not implemented in FLightR. Please, use appropriate functions in other R packages, e.g. BAStag, twGeos or GeoLight [Lisovski et al. 2012a](#lisovski_2012a), or online service http://tags.animalmigration.org for this purpose. In this software, user guides semi-automatically search for the twilight times and then visually explores the light data, manually removing twilights defined apparently wrong or around which the light pattern is atypical or systematically biased, e.g. when a bird enters or exits a cavity during twilight ([Rakhimberdiev et al. 2016](#rakhimberidev_jab_2016)). An example of the routine for the twilight definition in BAStag is available as [appendix A4](https://github.com/eldarrak/FLightR/blob/master/examples/Black-Tailed_Godwit_JAB_example/A4_BAStag_routine.Rmd) to [Rakhimberdiev et al. 2016](#rakhimberdiev_jab_2016).
...
We will use the TAGS formatted data provided as by Pedersen et al.:

_We need reference to Lykke here_

The original file is available [here](https://git.io/fAoHq) and the annotated file in tags format [here](https://git.io/fAoHs).

```{r, eval=FALSE, tidy=FALSE}
download.file('https://git.io/fAoHs', 'shrike_data_TAGS_format.csv')
...
```{r, eval=FALSE, tidy=FALSE}
FLightR.data<-get.tags.data("shrike_data_TAGS_format.csv")
```
...
```
The function produces an visual represnetation of the created grid with orange poins where bird is allowed to be stationary and grey, where bird can fly but not stay. Have a look at the figure and think twice if you gris is correct.
...
The function `make.prerun.object` creates a complex object, which will be used in the main run. It incorporates all the objects, created at earlier steps: the light data with the detected twilight events (`FLightR.data`), the spatial parameters (`Grid`), geographic coordinates of the initial location, where the tracking has started (`start`), and the calibration parameters (`Calibration`). Here one can also change the priors for the movement model. For example we set `M.mean` parameter to 750 km, because we know that shrikes migrate fast and also because we have explored the results and figured out that average migration distance was actually 750 km.

```{r, eval = F}
# ~ 15 min run time
all.in<-make.prerun.object(FLightR.data, Grid, start=c(12.23, 55.98), Calibration=Calibration, M.mean=750)
```
...
# `b` - the maximum distance allowed to be travelled between two consequtive twiloghts.
...
Result<-run.particle.filter(all.in, threads=-1,
            ...
            b=1700)
save(Result, file="./results/Result_1e6_long_calibration_model_ageing.RData")
```
Now the results are saved as an RData object.

## 6. First exploration - plot longitude and latitude

After the analysis finished and results are save the first thing to do is to plot change of longitude and latitude over time. We concider this graph being the most important.

```{r, eval = F}
plot_lon_lat(Result)
```
With this graph, please, carefully check whether:
1. Your track never approaches the edge of the spatial grid you defined. If this is the case, change spatial extend and rerun the analysis.
2. There is no sudden jumps that have no uncertainty measurements around them. These are likely outliers that should be excluded befoirehand.
3. Your resulting estiamates of time at the calibration site match the result. If not, correct the calibration periods and do the calibration again.
4. There is no S-shape patterns in estimated latitude around equinox. These point at either bad calibration or very bad data. Try to improve your calibration periods or concider takeing calibratio data from the other tag of the smae species.

## 7. Summary

Function `stationary.migration.summary` tries to distinguish when and where the bird was stationary and when did it move.

```{r, eval=FALSE}
# ~ 45 min run time
Summary<-stationary.migration.summary(Result, prob.cutoff = 0.2)
```
To visualise results of this function we will plot them over a map:
...
# And nwo I want to set up colors..
